...
public class Matrix extends GeneralSearch{
	...
	public static String genGrid() {
		...
		for (int i = 0; i < hostagesNum; i++) {
			...
			int damage = rand.nextInt(99 - 1) + 1;
			hostages += hos.x + "," + hos.y + ","  + damage;
		...
		int agentsNum = rand.nextInt(availableLocations.size() - 4) + 1;
		...
		int pillsNum = rand.nextInt(availableLocations.size() - 1)  + 1;
		...
		grid += ";";
		int padsNum = rand.nextInt(availableLocations.size() - 2) + 2;
		// check if the number is divisible by 2
		while (padsNum % 2 != 0) {
			// try again
			padsNum = rand.nextInt(availableLocations.size() - 2) + 2;				
		}
